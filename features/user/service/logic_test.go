...
func TestGetById(t *testing.T) {
	repo := new(mocks.UserRepository)
	returnData := user.Core{ID: 1, Name: "alta", Email: "alta@mail.id", Password: "qwerty", Phone: "081234", Address: "Jakarta", Role: "user"}
	t.Run("Success Get id data", func(t *testing.T) {
	...
	t.Run("Failed Get User by id", func(t *testing.T) {
		User := user.Core{}
		repo.On("GetById", 1).Return(User, errors.New("failed to get data by id")).Once()
		srv := New(repo)
		response, err := srv.GetById(1)
		assert.NotNil(t, err)
...
func TestDeleteUser(t *testing.T) {
	...
	t.Run("Success Delete User", func(t *testing.T) {
		repo.On("DeleteUser", 1).Return(1, nil).Once()
		srv := New(repo)
	...
	t.Run("Failed Delete User", func(t *testing.T) {
		repo.On("DeleteUser", 1).Return(0, errors.New("failed delete user, error query")).Once()
		srv := New(repo)
		err := srv.DeleteUser(1)
		assert.NotNil(t, err)
		assert.Equal(t, "failed delete user, error query", err.Error())
		repo.AssertExpectations(t)
	})
}
...
func TestUpdateUser(t *testing.T) {
	...
	inputData := user.Core{Name: "alta", Email: "alta@mail.id", Password: "qwerty", Phone: "081234", Address: "Jakarta"}
	t.Run("Success Get id data", func(t *testing.T) {
		repo.On("UpdateUser", inputData, 1).Return(nil).Once()
		srv := New(repo)
		err := srv.UpdateUser(inputData, 1)
		assert.NoError(t, err)
		repo.AssertExpectations(t)
	})

	t.Run("Failed Update User", func(t *testing.T) {
		repo.On("UpdateUser", inputData, 1).Return(errors.New("failed update data, error query")).Once()
		srv := New(repo)
		err := srv.UpdateUser(inputData, 1)
		assert.NotNil(t, err)
